...
Responsibility:
    ...
    Every statement node is tagged with the source line it starts on
    (see ast_nodes.py) so that semantic_analyzer.py can report clear
    diagnostics later without re-scanning tokens.
...
Note: this grammar only covers *syntax*. Whether a split's percentages
add up to 100, whether a model type is supported, etc. are semantic
questions, checked afterwards by semantic_analyzer.py.
"""
...
class Parser:
    ...
    def _parse_pipeline(self) -> Pipeline:
        start_tok = self._expect(TokenType.KEYWORD, "pipeline")
        ...
        self._expect(TokenType.SYMBOL, "}")
        return Pipeline(name=name_tok.value, statements=statements, line=start_tok.line)
    ...
    def _parse_statement(self):
        ...
        if tok.value == "train":
            self._advance()
            self._expect(TokenType.SYMBOL, ";")
            return TrainStmt(line=tok.line)
    ...
    def _parse_load(self) -> LoadStmt:
        start_tok = self._expect(TokenType.KEYWORD, "load")
        self._expect(TokenType.KEYWORD, "data")
        path_tok = self._expect(TokenType.STRING)
        self._expect(TokenType.SYMBOL, ";")
        return LoadStmt(path=path_tok.value, line=start_tok.line)

    def _parse_preprocess(self) -> PreprocessBlock:
        start_tok = self._expect(TokenType.KEYWORD, "preprocess")
        self._expect(TokenType.SYMBOL, "{")
        steps = []
        while not self._check(TokenType.SYMBOL, "}"):
            if self._at_end():
        ...
        self._expect(TokenType.SYMBOL, "}")
        return PreprocessBlock(steps=steps, line=start_tok.line)
    ...
    def _parse_preprocess_step(self):
        ...
        if tok.value == "fill_missing":
            return FillMissingStep(column=col_tok.value, line=tok.line)
        if tok.value == "scale":
            return ScaleStep(column=col_tok.value, line=tok.line)
        return EncodeStep(column=col_tok.value, line=tok.line)

    def _parse_split(self) -> SplitStmt:
        start_tok = self._expect(TokenType.KEYWORD, "split")
        ...
        self._expect(TokenType.SYMBOL, ";")
        return SplitStmt(
            train_pct=int(train_tok.value),
            test_pct=int(test_tok.value),
            line=start_tok.line,
        )

    def _parse_model(self) -> ModelBlock:
        start_tok = self._expect(TokenType.KEYWORD, "model")
        ...
        self._expect(TokenType.SYMBOL, "}")
        return ModelBlock(model_type=type_tok.value, params=params, line=start_tok.line)
    ...
    def _parse_param(self) -> ModelParam:
        ...
        self._expect(TokenType.SYMBOL, ";")
        return ModelParam(name=name_tok.value, value=int(value_tok.value), line=name_tok.line)

    def _parse_evaluate(self) -> EvaluateStmt:
        start_tok = self._expect(TokenType.KEYWORD, "evaluate")
        ...
        self._expect(TokenType.SYMBOL, "]")
        self._expect(TokenType.SYMBOL, ";")
        return EvaluateStmt(metrics=metrics, line=start_tok.line)
